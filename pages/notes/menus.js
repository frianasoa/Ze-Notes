Menus = {
	...
	actions (key, options)
    {
        ...
        else if(key=="hidecolumn")
        {
			...
			Zotero.ZeNotes.Database.hidecolumn(this.collectionid, column).then(()=>{
				Zotero.ZeNotes.Ui.reload();
			})
        }
    ...
    opennote(itemID, col, parentKey)
    {
		...
		var win = Zotero.ZeNotes.window.openDialog('chrome://zotero/content/note.xhtml', name, 'chrome,resizable,centerscreen,dialog=false', io);
