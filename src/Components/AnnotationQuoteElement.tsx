...
const AnnotationQuoteElement: React.FC<AnnotationSelectionElementProps> = ({ item, onContextMenu }) => {
  ...
  return (
    <div
      ...
      className="selection zcontent"
      data-legend="Direct quote"
      data-annotationid={item.annotationid}
      style={{ background: backgroundColor, border: "dotted 1px", padding: "0.3em" }}
    >
      <span className="annotation-quote">“{item.text}”</span>&nbsp;
      <span className="annotation-source" data-page={item.pagelabel}>
        {item.source}
      </span>
    </div>
  );
