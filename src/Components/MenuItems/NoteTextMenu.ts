...
const NoteTextMenu = {
  show(context: any, event: React.MouseEvent<HTMLElement, MouseEvent>, item: Record<string, any>)
  {
    ...
    // AI providers
    AiMenu.insertprompts(context, event, ["openai", "gemini", "deepseek", "claude", "customai"], [
      {key: "note", target: "note", label: "Prompt on note", icon: FaNoteSticky, data: { noteid: item.noteid }},
      {key: "notepart", target: "notepart", label: "Prompt on part", icon: FaNoteSticky, data: { title: "Prompt on "+title, noteid: item.noteid }},
    ]);
