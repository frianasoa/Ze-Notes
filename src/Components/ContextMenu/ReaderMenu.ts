...
const ReaderMenu = {
  ...
  async updateannotation(annotation: Zotero.Item, title: string, text_: string) {
    ...
    annotation.saveTx({ skipSelect: false }).catch((e: any) => {
      throw e;
    });
  },
  ...
  async addgroup(event: AnnotationContextMenuEvent, menus: ReaderMenuEntry[]) {
    ...
    for (const menu of menus) {
      ...
      for (const id of params.ids) {
        annotation = reader._item.getAnnotations().filter(function (e: Zotero.Item) {
          return e.key == id;
        })[0];
      }
  ...
  async addmainai() {
    ...
    if (models.data.length > 0) {
      menus.push({
        ...
        callback: (annotation) => {
          const text = annotation.annotationText;
          OpenAI.prompt(JSON.stringify({ quote: text }))
            .then((res: string[]) => {
    ...
    if (ZPrefs.get("custom-ai-url").trim()) {
      menus.push({
        ...
        callback: (annotation) => {
          const text = annotation.annotationText;
          CustomAI.prompt(JSON.stringify({ quote: text }), "custom-ai")
            .then((res: string[]) => {
  ...
  async addcustomai() {
    ...
    for (const key of Object.keys(settings).sort()) {
      ...
      menus.push({
        ...
        callback: (annotation) => {
          const text = annotation.annotationText;
          CustomAI.prompt(JSON.stringify({ quote: text }), key)
            .then((res: string[]) => {
