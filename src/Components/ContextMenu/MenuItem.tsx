...
const MenuItem: React.FC<MenuItemProps> = ({
  ...
  const [submenuPosition, setSubmenuPosition] = useState<{
    ...
    position: Position;
    overflow: boolean;
  }>({ left: "101%", right: "100%", top: undefined, position: "absolute", overflow: false });
      ...
      <li className={styles.li}>
        <label title={data.title || data.label}>
          ...
          <input
            ...
            value={elementValues[data.options.key] ?? ""}
    ...
    >
      ...
      {!data.icon && (
        <div
          ...
          style={{ color: data.iconColor, marginRight: "8px", width: "16px", height: "16px" }}
        ></div>
      )}

      {data.submenu ? (
        <span style={{ display: "flex", width: "100%", gap: "0.1em" }}>
          <span style={{ flex: "1" }} title={data.title || data.label}>
            {data.label}{" "}
          </span>
          <span style={{ color: "lightgray" }}>&gt;</span>
        </span>
      ) : (
        <span className={styles.label} style={{ color: data.textColor }} title={data.title || data.label}>
          {data.label}
        </span>
      )}
