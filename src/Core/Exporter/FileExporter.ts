...
const FileExporter = {
  ...
  async exportdone(obj: any, worked: boolean) {
    ...
    if (!worked) {
      Zotero.alert(window, Zotero.getString("general.error"), Zotero.getString("fileInterface.exportError"));
    }
  },
  ...
  async exportdata(collection: any, email: string) {
    ...
    const translator = (await translation.getTranslators()).find((t: any) => t.translatorID === zoteroRDFTranslatorID);
