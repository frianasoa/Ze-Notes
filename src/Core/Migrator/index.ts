...
const Migrator = {
  ...
  async createcustom() {
    ...
    if (openaikey) {
      ...
      options =
        '{ "method": "${method}", "headers": { "Content-Type": "application/json" }, "body": { "model": "${model}", "system_instruction": { "parts": [ { "text": "${systemmessage}" } ] }, "contents": [ { "parts": [ { "text": "${userprompt}" }, { "text": "${text}" } ] } ], "messages": [ { "role": "system", "content": "${systemmessage}" }, { "role": "user", "content": "${userprompt}" }, { "role": "user", "content": "${text}" } ] } }';
      format =
        "(data) => {       if (data.candidates && data.candidates.length > 0) {         return data.candidates.map((e) => e.content?.parts?.map((p) => p.text).join('') || '');       }       return [];}";
