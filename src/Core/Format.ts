...
const Format = {
  zpaths(item: _ZoteroTypes.Items): string {
    ...
    return item.getAttachments().map((id: number) => {
      ...
      if (library.isGroup) {
        return {
          type: "file",
          value: "zotero://open-pdf/library/" + item.libraryID + "/item/" + attachment.key,
          key: attachment.key,
          filetype: attachment.attachmentContentType,
          id: attachment.id,
        };
      } else {
        return {
          type: "file",
          value: "zotero://open-pdf/library/items/" + attachment.key,
          key: attachment.key,
          filetype: attachment.attachmentContentType,
          id: attachment.id,
        };
  ...
  date(item: _ZoteroTypes.Items): string {
    const date =
      ...
      item.getField("dateDecided", true) ||
      "n.d ";
