...
const STANDARD_LOADING =
  "Loading, please wait...<br/> Or close this window, <br/> and continue working while waiting for a note window to open!";
const CUSTOM_LOADING =
  "Loading, please wait...<br/> Or close this window if you want, <br/> and continue working while waiting for a note window to open!";
...
const AiPrompt = {
  ...
  resolvetarget(
    ...
    custom: Record<string, TargetResolver | null> = {},
  ): HTMLElement | null {
  ...
  async run(
    promptfn: (data: string) => Promise<any>,
    ...
    const makecontents =
      ...
      ((data: any, item: zty.ContextMenuData) => {
    ...
    promptfn(JSON.stringify(promptdata))
      .then((data: any) => {
        AiNotes.create(item, celldata, makecontents(data, item), () => {
          Reload.reload();
        });
  ...
  async runstandard(
    provider: { prompt(data: string): Promise<any> },
    item: zty.ContextMenuData,
    celldata: zty.CellData,
  ) {
