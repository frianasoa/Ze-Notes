...
const CustomAi = {
  async params(key: string) {
    ...
    if (key == "custom-ai") {
      ...
      data["systemmessage"] = ZPrefs.get(
        "custom-ai-system-message",
        "You are an academic assistant helping in literature review.",
      );
      data["userprompt"] = ZPrefs.get(
        "custom-ai-user-prompt",
        "Summarize the following information (the format is json).",
      );
  ...
  applyvar(obj: any, params: Record<string, string>): any {
    if (typeof obj === "string") {
      ...
      return Object.fromEntries(Object.entries(obj).map(([key, value]) => [key, CustomAi.applyvar(value, params)]));
    }
    return obj;
  },
  ...
  async settinglist() {
    ...
    return Object.fromEntries(Object.entries(settings).map(([key, value]) => [key, JSON.parse(value)]));
  },
